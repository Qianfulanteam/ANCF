...
import bookMF as movieMF
import bookMLP as movieMLP
import os
...
def get_lCoupledCF_model(num_users, num_items):
    ...
    #mf attr part
    user_mf_attr_embedding1 = Dense(1500, activation="relu",name="user_att_embedding_mf1")(user_attr_input)
    user_mf_attr_embedding = Dense(80, activation="relu",name="user_att_embedding_mf")(user_mf_attr_embedding1)  # 1st hidden layer
    attention_probs_u_mf = Dense(80, activation='softmax', name='attention_probs_u_mf')(user_mf_attr_embedding)
    attention_u_mf = multiply([user_mf_attr_embedding, attention_probs_u_mf], name='attention_u_mf')
    z_u_mf = Dense(40, activation="relu", name='z_u_embedding_mf1')(attention_u_mf)  # z
    item_mf_attr_embedding1 = Dense(80, activation="relu",name="item_att_embedding_mf1")(item_attr_input)
    item_mf_attr_embedding = Dense(80, activation="relu",name="item_att_embedding_mf")(item_mf_attr_embedding1)  # 1st hidden layer
    attention_probs_i_mf = Dense(80, activation='softmax', name='attention_probs_i_mf')(item_mf_attr_embedding)
    attention_i_mf = multiply([item_mf_attr_embedding, attention_probs_i_mf], name='attention_i_mf')
    z_i_mf = Dense(40, activation="relu", name='z_i_embedding_mf')(attention_i_mf)  # z

    # mlp attr part
    user_mlp_attr_embedding1 = Dense(1500, activation="relu",name="user_att_embedding_mlp1")(user_attr_input)
    user_mlp_attr_embedding = Dense(80, activation="relu",name="user_att_embedding_mlp")(user_mlp_attr_embedding1)  # 1st hidden layer
    attention_probs_u_mlp = Dense(80, activation='softmax', name='attention_probs_u_mlp')(user_mlp_attr_embedding)
    attention_u_mlp = multiply([user_mlp_attr_embedding, attention_probs_u_mlp], name='attention_u_mlp')
    z_u_mlp = Dense(40, activation="relu", name='z_u_embedding_mlp')(attention_u_mlp)  # z
    item_mlp_attr_embedding1 = Dense(900, activation="relu",name="item_att_embedding_mlp1")(item_attr_input)
    item_mlp_attr_embedding = Dense(80, activation="relu",name="item_att_embedding_mlp")(item_mlp_attr_embedding1)  # 1st hidden layer
    attention_probs_i_mlp = Dense(80, activation='softmax', name='attention_probs_i_mlp')(item_mlp_attr_embedding)
    attention_i_mlp = multiply([item_mlp_attr_embedding, attention_probs_i_mlp], name='attention_i_mlp')
    z_i_mlp = Dense(40, activation="relu", name='z_i_embedding_mlp')(attention_i_mlp)  # z
    ...
    layer = Dense(72, activation='relu', name='layer2')
    vector = layer(predict_vector_mlp)
    layer = Dense(36, activation='relu', name='layer3')
    vector = layer(vector)
    layer = Dense(18, activation='relu', name='layer4')
...
def load_pretrain_model(model, gmf_model, mlp_model):
    # MF embeddings
    gmf_user_att_embeddings1 = gmf_model.get_layer('user_att_embedding1').get_weights()
    ...
    gmf_item_att_embeddings1 = gmf_model.get_layer('item_att_embedding1').get_weights()
    ...
    model.get_layer('user_att_embedding_mf1').set_weights(gmf_user_att_embeddings1)
    ...
    model.get_layer('item_att_embedding_mf1').set_weights(gmf_item_att_embeddings1)
    ...
    mlp_user_att_embeddings1 = mlp_model.get_layer('user_att_embedding1').get_weights()
    ...
    mlp_item_att_embeddings1 = mlp_model.get_layer('item_att_embedding1').get_weights()
    ...
    model.get_layer('user_att_embedding_mlp1').set_weights(mlp_user_att_embeddings1)
    ...
    model.get_layer('item_att_embedding_mlp1').set_weights(mlp_item_att_embeddings1)
...
def main():
    ...
    dataset="book"
    num_factor=32
    #mf_pretrain = "Pretrain/ml_1m_movieMF_32_1596216601.h5"
    #mlp_pretrain = "Pretrain/ml_1m_movieMLP_8_1596291564.h5"
    mf_pretrain = ""
    mlp_pretrain = ""
    evaluation_threads = 1
    startTime = time()
    model_out_file = 'Pretrain/%s_bookCF8n_%d_%s_%d.h5' %(dataset, num_factor, 5, time())
    # load data
    ...
    if out > 0:
        print("The best bookCF model is saved to %s" %(model_out_file))
