...
def get_lCoupledCF_model(num_users, num_items):
    ...
    user_attr_embedding = Dense( 16, activation="relu",name="user_att_embedding")(user_attr_input)  # 1st hidden layer


    attention_probs_u = Dense(16, activation='softmax', name='attention_probs_u')(user_attr_embedding)
...
def main():
    ...
    #num_negatives = 4
    startTime = time()
    model_out_file = 'Pretrain/%s_movieMF_%d_%d.h5' % (dataset,num_factor , time())
    # load data
    ...
    if out > 0:
        print("The best movieMF model is saved to %s" %(model_out_file))
