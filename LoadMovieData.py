...
def load_rating_file_as_list():
    ratingList = []
    with open('ml-1m/ml-1m.test.rating', "r") as f:
        line = f.readline()
...
def load_negative_file():
    negativeList = []
    with open('ml-1m/ml-1m.test.negative', "r") as f:
        line = f.readline()
        while line != None and line != "":
            arr = line.split("\t")
            negatives = []
...
def load_itemGenres_as_matrix():
    ...
    with open("ml-1m/u.genre", "r") as f:
        line = f.readline().strip('\n')
        while line != None and line != "":
            arr = line.split("|")
            dict[arr[0]] = num_type
