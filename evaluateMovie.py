...
def evaluate_model(model, testRatings, testNegatives, userAttrMat, itemAttrMat, K, num_thread):
    ...
    if (num_thread > 1):  # Multi-thread
        ...
        recalls = [r[2] for r in res]
        return (hits, ndcgs, recalls)
    # Single thread
    for idx in range(len(_testRatings)):
        (hr, ndcg, recall) = eval_one_rating(idx)
        hits.append(hr)
        ndcgs.append(ndcg)
...
def eval_one_rating(idx):
    ...
    map_item_score = {}
    predictions = _model.predict([user_attr_input_mat, item_attr_input_mat, user_id_input_mat, item_id_input_mat],
    ...
    recall = getRecall(ranklist, gtItem)
