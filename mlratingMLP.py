...
def get_lCoupledCF_model(num_users, num_items):
    ...
    vector = layer(vector)
    # layer = Dense(6, W_regularizer=l2(0), activation='relu', name='layer4')
    # vector = layer(vector)
    # layer = Dense(3, W_regularizer=l2(0), activation='relu', name='layer5')
    # vector = layer(vector)
...
def main():
    ...
    startTime = time()
    model_out_file = 'Pretrain/%s_movieMLP100k_%d_%d.h5' % (dataset,num_factor , time())
    # load data
    ...
    if out > 0:
        print("The best movieMLP100k model is saved to %s" %(model_out_file))
