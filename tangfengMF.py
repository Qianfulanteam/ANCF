...
def get_lCoupledCF_model(num_users, num_items):
    ...
    merge_attr_id_embedding = multiply([user_id_attr, item_id_attr])
...
def main():
    ...
    out=1
    startTime = time()
    model_out_file = 'Pretrain/%s_tafengMF_%d_%d.h5' % (dataset, num_factor, time())
    # load data
    ...
    if out > 0:
        print("The best tafengMF model is saved to %s" %(model_out_file))
